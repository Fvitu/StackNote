...
function writeCache(entries: CachedNote[]) {
	...
	window.localStorage.setItem(NOTE_CACHE_KEY, JSON.stringify(entries.slice(0, NOTE_CACHE_LIMIT)));
}
...
function buildEntry(input: CacheableNoteInput, existing?: CachedNote): CachedNote {
	return {
		id: input.id,
		title: input.title,
