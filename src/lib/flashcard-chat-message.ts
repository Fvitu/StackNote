...
export function serializeFlashcardDeckMessage(deck: FlashcardDeckPayload) {
  return [
    `Generated ${deck.count} flashcards for "${deck.title}".`,
    `${FLASHCARD_MESSAGE_OPEN}${JSON.stringify(deck)}${FLASHCARD_MESSAGE_CLOSE}`,
  ].join("\n\n")
}
...
export function parseFlashcardDeckMessage(content: string): FlashcardDeckPayload | null {
  ...
  const serialized = content
    .slice(start + FLASHCARD_MESSAGE_OPEN.length, end)
    .trim()
  ...
  try {
    ...
    return {
      ...
      cards: parsed.cards.filter(
        (card): card is FlashcardDeckPayload["cards"][number] =>
          Boolean(
            card &&
