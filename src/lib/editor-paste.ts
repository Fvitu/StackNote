...
function findLanguageAlias(value: string): SupportedLanguageId | undefined {
...
function getLanguageHintFromLabel(text: string): SupportedLanguageId | undefined {
...
function stripAttachedLanguagePrefix(text: string): { code: string; hint?: SupportedLanguageId } {
  ...
  for (const entry of LANGUAGE_ALIASES) {
    if (!normalized.toLowerCase().startsWith(entry.alias)) {
      continue
    }

    const remainder = normalized.slice(entry.alias.length)
