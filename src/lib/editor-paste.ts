...
function stripAttachedLanguagePrefix(text: string): { code: string; hint?: SupportedLanguageId } {
	...
	for (const alias of normalized
		...
		if (
			/^(from\s|import\s|class\s|def\s|const\s|let\s|var\s|function\s|package\s|func\s|fn\s|SELECT\b|INSERT\b|UPDATE\b|DELETE\b|CREATE\b|ALTER\b|DROP\b|<\/?[a-z]|\{|\[|\()/i.test(
				remainder,
			)
		) {
			return {
