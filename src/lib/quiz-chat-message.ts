...
export function parseQuizHistoryMessage(content: string): QuizHistoryPayload | null {
  ...
  const serializedPayload = content
    .slice(start + QUIZ_MESSAGE_OPEN.length, end)
    .trim()
  ...
  try {
    ...
    const parsedQuestions = Array.isArray(parsedPayload.questions)
      ? parsedPayload.questions.filter(isQuizQuestion)
      : []
