...
export function PomodoroTimer({
	...
	return (
		<div
			className="rounded-[24px] border px-5 py-4 shadow-[0_24px_60px_rgba(0,0,0,0.35)]"
			style={{ backgroundColor: "rgba(10,10,10,0.94)", borderColor: `${accentColor}40`, backdropFilter: "blur(14px)" }}>
			...
			<div className="mt-4 grid grid-cols-3 gap-2">
				<button
					...
					className="inline-flex items-center justify-center gap-1 rounded-xl px-3 py-2 text-sm font-medium transition-colors"
					style={{ backgroundColor: "var(--accent-muted)", color: accentColor }}>
				...
				<button
					type="button"
					onClick={onReset}
					className="inline-flex items-center justify-center gap-1 rounded-xl border px-3 py-2 text-sm transition-colors hover:bg-[var(--bg-hover)]"
...
function PomodoroSettingsPanel({
	...
	return (
		<div
			className="rounded-[24px] border px-5 py-4 shadow-[0_24px_60px_rgba(0,0,0,0.35)]"
			style={{ backgroundColor: "rgba(10,10,10,0.94)", borderColor: "var(--border-default)", backdropFilter: "blur(14px)" }}>
			<div className="flex items-center justify-between gap-3">
				<p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>
					Timer settings
				</p>
			...
			<div className="mt-4 space-y-3">
				...
				<label className="flex items-center justify-between gap-3 text-sm">
					...
					<input
						...
						onChange={(event) => setDraft((previousDraft) => ({ ...previousDraft, autoStartBreaks: event.target.checked }))}
					/>
				</label>
				...
				<label className="flex items-center justify-between gap-3 text-sm">
					...
					<input
						...
						onChange={(event) => setDraft((previousDraft) => ({ ...previousDraft, autoStartPomodoros: event.target.checked }))}
					/>
				</label>
			</div>
			...
			<button
				...
				className="mt-4 w-full rounded-xl px-3 py-2 text-sm font-medium text-white"
