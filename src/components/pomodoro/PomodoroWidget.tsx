...
export function PomodoroWidget({ sidebarOffset }: PomodoroWidgetProps) {
	...
	useEffect(() => {
		const updateMobileScreenState = () => {
			setIsMobileScreen(window.innerWidth < 768);
		};

		updateMobileScreenState();
		window.addEventListener("resize", updateMobileScreenState);

		return () => {
			window.removeEventListener("resize", updateMobileScreenState);
		};
	}, []);
	...
	return createPortal(
		<div
			ref={containerRef}
			className="pointer-events-none fixed bottom-6 z-[5] w-[300px]"
			style={{
