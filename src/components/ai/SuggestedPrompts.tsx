...
export function SuggestedPrompts({ onSelect, noteTitle }: SuggestedPromptsProps) {
	return (
		<div className="flex flex-col items-center justify-center py-8">
			<div className="flex h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: "var(--accent-muted)" }}>
				<Sparkles className="h-6 w-6" style={{ color: "var(--sn-accent)" }} />
			</div>

			<h3 className="mt-4 text-center text-sm font-medium" style={{ color: "var(--text-primary)" }}>
				AI Assistant
			</h3>

			<p className="mt-1 text-center text-xs" style={{ color: "var(--text-tertiary)" }}>
				{noteTitle ? `Ask questions about "${noteTitle}"` : "Ask questions or get help with your notes"}
			</p>
