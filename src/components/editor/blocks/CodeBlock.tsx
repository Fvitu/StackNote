...
function getCodeBlockPropsFromElement(element: HTMLElement) {
	...
	const language = detectCodeLanguage(code, codeElement.getAttribute("data-language") ?? codeElement.className ?? undefined);
		...
		render: (props) => {
			...
			useEffect(() => {
				const onClickOutside = (event: MouseEvent) => {
					...
					if (isEditing && codeRef.current && !codeRef.current.contains(event.target as Node)) {
			...
			if (isPreview) {
				return (
					<BlockContentWrapper
						...
						<div
							className="w-full overflow-hidden rounded-[var(--sn-radius-lg)] border"
							...
							<div
								...
								<div className="flex items-center gap-2">
									<span
										className="rounded-[3px] px-1 py-0.5 text-[10px]"
										style={{ backgroundColor: "rgba(124,106,255,0.2)", color: "#c4bcff" }}>
							...
							<div className="flex">
								...
								<div
									className="min-w-0 flex-1 overflow-x-auto px-5 py-4 stacknote-shiki"
					...
					<div
										...
										<div className="max-h-64 overflow-y-auto py-1">
											{filteredLanguages.map((language) => (
												<button
													...
													{language.id === props.block.props.language && (
														<Check className="h-3.5 w-3.5" style={{ color: "var(--sn-accent)" }} />
													)}
												</button>
											))}
										</div>
						...
						<div ref={codeRef} className="relative transition-opacity duration-150" style={{ opacity: loading ? 0.75 : 1 }}>
							{isEditing ? (
								...
								<button type="button" className="block w-full text-left" onClick={() => setIsEditing(true)}>
									<div className="flex">
										{props.block.props.showLineNumbers && (
											<div
												className="select-none px-3 py-4 text-right text-[13.5px]"
		...
		toExternalHTML: (props) => (
			<pre data-filename={props.block.props.filename || undefined}>
				<code
					...
					data-filename={props.block.props.filename || undefined}
