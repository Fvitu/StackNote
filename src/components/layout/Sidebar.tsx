...
interface SidebarProps {
  ...
  onCreateNote?: (folderId?: string) => Promise<any>
  onCreateFolder?: (parentId?: string) => Promise<any>
  onDeleteNote?: (noteId: string) => Promise<any>
  onDeleteFolder?: (folderId: string) => Promise<any>
  onRenameNote?: (noteId: string, title: string) => Promise<any>
  onRenameFolder?: (folderId: string, name: string) => Promise<any>
  onMoveNote?: (noteId: string, folderId: string | null) => Promise<any>
  onMoveFolder?: (folderId: string, parentId: string | null) => Promise<any>
}
...
export function Sidebar({
  ...
  const { state, setActiveNote, toggleFolder, toggleSidebar, setFullscreen } = useWorkspace()
  ...
  const handleDragStart = useCallback(
    (_e: React.DragEvent, id: string, type: "note" | "folder") => {
  ...
  const handleDragOverFolder = useCallback(
    (_e: React.DragEvent, folderId: string) => {
    ...
    return (
      <div key={folder.id} className="item-enter">
        <SidebarItem
          ...
          draggable
          onDragStart={(e) =>
            (handleDragStart as any)(e, folder.id, "folder")
          }
          menuActions={{
        ...
        >
          ...
          {folder.notes.map((note) => (
            <SidebarItem
              ...
              draggable
              onDragStart={(e) =>
                (handleDragStart as any)(e, note.id, "note")
              }
              menuActions={{
                onChangeIcon: () => {
                  setActiveNote(note.id)
                  window.dispatchEvent(
                    new CustomEvent("open-emoji-picker", { detail: { noteId: note.id } })
                  )
                },
    ...
    >
      ...
      <div className="space-y-1 p-2">
        ...
        >
          ...
          Search
          <span className="ml-auto text-xs text-[#555555]">⌘K</span>
        </Button>
      </div>
