...
export function SettingsPanel({ variant, onClose }: SettingsPanelProps) {
  ...
  const rootClassName = variant === "dock" ? "flex h-full flex-col overflow-hidden" : "flex h-full flex-col overflow-y-auto"
  const contentClassName = variant === "dock" ? "flex-1 overflow-y-auto px-4 py-6" : "mx-auto w-full max-w-2xl px-6 py-8"
          ...
          {loadingUsage ? (
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={group.title} className="rounded-lg border p-4" style={{ backgroundColor: "var(--bg-surface)", borderColor: "var(--border-default)" }}>
                    ...
                    {group.items.map((item) => {
                      ...
                      const flashcardMetric =
                        ...
                        (isStudyTemplates
                          ? {
                              used: 0,
                              limit: FLASHCARD_MODEL_LIMITS[item.model as keyof typeof FLASHCARD_MODEL_LIMITS]?.flashcardsPerWindow ?? 0,
                              remaining: FLASHCARD_MODEL_LIMITS[item.model as keyof typeof FLASHCARD_MODEL_LIMITS]?.flashcardsPerWindow ?? 0,
                            }
                          : null)
                      const questionMetric =
                        ...
                        (isStudyTemplates
                          ...
                          : null)
                      const secondaryMetric = isStudyTemplates ? null : item.tokens ?? item.flashcards ?? item.questions ?? item.audioSeconds ?? null
                      ...
                      return (
                        <div key={`${item.category}-${item.model}`} className="rounded-md border p-3" style={{ borderColor: "var(--border-default)" }}>
                          <div className="mb-2 flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>
                                {item.resetAt ? `Window resets in ${formatUsageResetCountdown(item.resetAt, now)}` : "Window starts on the first request."}
                              </p>
                            </div>
                            <div className="text-right text-xs" style={{ color: "var(--text-secondary)" }}>
                              ...
                              {isStudyTemplates ? (
                                <>
                                  <div>Flashcards: {flashcardMetric?.used ?? 0} / {flashcardMetric?.limit ?? 0}</div>
                                  <div>Questions: {questionMetric?.used ?? 0} / {questionMetric?.limit ?? 0}</div>
                                </>
                              ) : (
                                <>
                                  {item.tokens ? <div>Tokens: {item.tokens.used.toLocaleString()} / {item.tokens.limit.toLocaleString()}</div> : null}
                                  {item.flashcards ? <div>Flashcards: {item.flashcards.used} / {item.flashcards.limit}</div> : null}
                                  {item.questions ? <div>Questions: {item.questions.used} / {item.questions.limit}</div> : null}
                                  {item.audioSeconds ? <div>Audio: {formatDurationSeconds(item.audioSeconds.used)} / {formatDurationSeconds(item.audioSeconds.limit)}</div> : null}
                                </>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            <div>
                              ...
                              <div className="h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: "var(--bg-hover)" }}>
                                <div className="h-full transition-all" style={{ width: `${metricPercentage(item.requests)}%`, backgroundColor: "var(--sn-accent)" }} />
                              </div>
                            </div>
                            ...
                            {isStudyTemplates ? (
                              <>
                                <div>
                                  ...
                                  <div className="h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: "var(--bg-hover)" }}>
                                    <div className="h-full transition-all" style={{ width: `${metricPercentage(flashcardMetric ?? { used: 0, limit: 1, remaining: 1 })}%`, backgroundColor: "var(--sn-accent)" }} />
                                  </div>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div className="h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: "var(--bg-hover)" }}>
                                    <div className="h-full transition-all" style={{ width: `${metricPercentage(questionMetric ?? { used: 0, limit: 1, remaining: 1 })}%`, backgroundColor: "var(--sn-accent)" }} />
                                  </div>
                                </div>
                              </>
                            ) : null}
                            ...
                            {secondaryMetric && secondaryLabel ? (
                              <div>
                                ...
                                <div className="h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: "var(--bg-hover)" }}>
                                  <div className="h-full transition-all" style={{ width: `${metricPercentage(secondaryMetric)}%`, backgroundColor: "var(--sn-accent)" }} />
                                </div>
                              </div>
                            ) : null}
                          </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(TEXT_MODEL_LIMITS).map(([modelId, limit]) => (
                    <p key={modelId}>
                      <strong style={{ color: "var(--text-primary)" }}>{limit.label}</strong>: {limit.requestsPerWindow} requests and {limit.tokensPerWindow.toLocaleString()} tokens
                    </p>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(STT_MODEL_LIMITS).map(([modelId, limit]) => (
                    <p key={modelId}>
                      <strong style={{ color: "var(--text-primary)" }}>{limit.label}</strong>: {limit.requestsPerWindow} requests and {formatDurationSeconds(limit.audioSecondsPerWindow)} of audio
                    </p>
                  ))}
                </div>
              </div>
