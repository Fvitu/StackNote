...
export async function GET(request: NextRequest) {
	...
	if (!accessKey) {
		return NextResponse.json({ error: "Unsplash is not configured. Add UNSPLASH_ACCESS_KEY to enable cover search." }, { status: 503 });
	}
	...
	const rawResults = payload && !Array.isArray(payload) && Array.isArray(payload.results) ? payload.results : Array.isArray(payload) ? payload : [];
