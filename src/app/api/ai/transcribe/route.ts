...
export async function POST(req: NextRequest) {
	...
	// Validate file type
	const validTypes = ["audio/mp3", "audio/mpeg", "audio/mpga", "audio/m4a", "audio/wav", "audio/webm", "audio/ogg", "video/mp4", "video/mpeg", "video/webm"];
	...
	try {
		...
		// Extract segments
		const segments: TranscriptSegment[] = (transcription.segments ?? []).map((seg: { start: number; end: number; text: string }) => ({
			start: seg.start,
			end: seg.end,
			text: seg.text.trim(),
		}));
		...
		return NextResponse.json({ error: "Transcription failed", message: String(error) }, { status: 500 });
	}
}
