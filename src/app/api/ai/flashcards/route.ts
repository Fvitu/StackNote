...
function parseGeneratedDeck(responseText: string, fallbackTitle: string): GeneratedDeck {
  ...
  const cardsSource = Array.isArray(payload.cards)
    ? payload.cards
...
async function requestFlashcardsFromModel(
  ...
  const promptSections = [
    ...
    targetLanguage
      ? `Write every flashcard and the deck title in ${targetLanguage}. Keep formulas, symbols, and technical terms accurate.`
      : "",
    ...
    const generation = await requestFlashcardsFromModel(PRIMARY_FLASHCARD_MODEL, text, targetCount, {
      ...
      if (!fallbackQuota.allowed) {
        throw new QuotaExceededError(
          fallbackQuota.error ?? "Flashcard limit reached",
          FALLBACK_FLASHCARD_MODEL,
