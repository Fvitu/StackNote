...
const ACTION_PROMPTS: Record<ActionType, string | ((lang?: string) => string)> = {
	summarize: "Summarize the following text concisely in bullet points. Keep key terms and main ideas. Return only the summary, no introduction.",
	expand: "Expand the following note snippet with more detail, examples, and explanations. Maintain the same tone and style. Return only the expanded content.",
	...
	translate: (lang?: string) => `Translate the following text to ${lang ?? "English"}. Return only the translation, no explanation.`,
	simplify: "Rewrite this text for a beginner student. Use simpler language, shorter sentences, and helpful analogies. Return only the simplified text.",
...
export async function POST(req: NextRequest) {
	...
	if (!action || !Object.keys(ACTION_PROMPTS).includes(action)) {
		return NextResponse.json({ error: "Invalid action", validActions: Object.keys(ACTION_PROMPTS) }, { status: 400 });
	}
	...
	if (action === "translate" && !targetLanguage) {
		return NextResponse.json({ error: "Target language is required for translate action" }, { status: 400 });
	}
	...
	try {
		...
		return NextResponse.json({ error: "AI service unavailable", message: String(error) }, { status: 500 });
	}
}
