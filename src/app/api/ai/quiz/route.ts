...
import { AI_LIMITS, FALLBACK_QUIZ_MODEL, PRIMARY_QUIZ_MODEL, type QuizQuotaModelId } from "@/lib/ai-limits";
...
async function generateQuizWithRetries(model: QuizQuotaModelId, promptInput: QuizGenerationPromptInput, questionCount: number) {
	...
	for (let attempt = 1; attempt <= MAX_QUIZ_GENERATION_ATTEMPTS; attempt += 1) {
		...
		try {
			...
			const attemptErrorMessage = attemptError instanceof Error ? attemptError.message.slice(0, 280) : "Unknown quiz generation error";
...
function validateQuestion(input: unknown): input is QuizQuestion {
	...
	return (
		...
		question.options.every(
			(option) =>
				option && typeof option === "object" && typeof option.id === "string" && typeof option.text === "string" && validOptionIds.includes(option.id),
		) &&
...
export async function POST(request: NextRequest) {
	...
	const customInstructions = typeof body.customInstructions === "string" ? body.customInstructions.trim().slice(0, 300) : "";
	...
	if (!primaryQuota.allowed) {
		return quotaExceededResponse(primaryQuota.error ?? "Question generation limit reached", PRIMARY_QUIZ_MODEL, primaryQuota.resetAt);
	}
	...
	try {
		...
		if (!usageResult.allowed) {
			return quotaExceededResponse(usageResult.error ?? "Question generation limit reached", generation.model, usageResult.resetAt);
		}
