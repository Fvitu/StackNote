...
export async function POST(req: NextRequest) {
  ...
  const { message, sessionId, noteId, noteContent, noteTitle, contextNoteIds, model, source } = body
  ...
  const effectiveContextNoteIds = activeSession.contextNoteIds.length > 0 ? activeSession.contextNoteIds : requestedContextNoteIds
  ...
  const noteContextText = buildContextText(contextNotes)
  const contextLabel = buildContextLabel(contextNotes)
  const persistedNoteId = noteId && contextNotes.some((contextNote) => contextNote.id === noteId) ? noteId : null
    ...
    async start(controller) {
      ...
      try {
        ...
        if (totalTokens > 0) {
          ...
          if (inputTokens === 0 && outputTokens === 0) {
            inputTokens = Math.min(totalTokens, Math.ceil(message.length / 4))
            outputTokens = Math.max(totalTokens - inputTokens, 0)
          }
        } else {
        ...
        await prisma.aIChatSession.update({
          ...
          data: {
            ...
            ...(activeSession.contextNoteIds.length === 0 && effectiveContextNoteIds.length > 0
              ? { contextNoteIds: effectiveContextNoteIds }
              : {}),
            ...(existingMessageCount === 0 ? { title: buildChatSessionTitle(message) } : {}),
          },
        })
